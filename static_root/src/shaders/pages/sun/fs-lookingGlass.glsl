...
uniform vec3 lookingGlassColor;
uniform sampler2D lookingGlassSampler;
...
void main() {
  ...
  vec3 nNormal = normalize( vNormalMat * vNormal );

  float fr = dot( normalize(vEye) , nNormal );



  vec4 color = texture2DProj(lookingGlassSampler, lookingGlassCoord +vec4( nNormal*50. ,0.));
  color = vec4(blendOverlay(lookingGlassColor.r, color.r), blendOverlay(lookingGlassColor.g, color.g), blendOverlay(lookingGlassColor.b, color.b), 1.0);
  ...
  gl_FragColor = color;//(1. -  d*d*d*d*d) * (2. *c1 + c2);// + c3;
