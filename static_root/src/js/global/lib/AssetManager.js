...
import loadImages from './image-promise/index.js'
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js'
import loadTexture from './loadTexture.js'
import loadEnvMap from './loadEnvMap.js'

const ORIGIN_URL = window.location.origin;

const ASSET_URL = ORIGIN_URL + "/static/src/assets/";
...
class AssetManager {
  ...
  async _loadItem({ url, type, renderer, ...options }) {
    ...
    switch (type) {
      ...
      case 'svg':
      case 'image':
        return loadImages(url, { crossorigin: 'anonymous' })
      case 'texture':
