...
void main() {
  ...
  //set final render
  if(opacity1>0.0){
    gl_FragColor = vec4(rgbcolor2,0.)*vec4(1.0);
...
const fragcylinder = `
    ...
    void main() {
        ...
        if(gl_FragColor.r >= 0.5){
            gl_FragColor = vec4(vec3(0.5, 0.5, 0.5),gl_FragColor.r);
        }else{
            gl_FragColor = vec4(0.);
        }
...
const fragflame = `
   #define GLSLIFY 1
    ...
    void main() {
        ...
        vec3 customColor = brightnessToColor(brightness) * alpha;

        vec3 finalColor = normalize(customColor * rgbcol(color4));
        gl_FragColor = vec4(noisetex.r);

        if(gl_FragColor.r >= 0.44){
            gl_FragColor = vec4(finalColor,gl_FragColor.r);
        }
...
ParticleEngine.prototype.destroy = function(group)
{
 ...
 class SpaceShift extends GTLFCustomLoader{
  ...
  addObjects() {
      this.addSingleObject(this.mdSettings[0])
      .then((model) => {
        this.model1 = model.scene;
        this.group.add(this.model1);
        this.group.scale.set(this.mdSettings[0].scale, this.mdSettings[0].scale, this.mdSettings[0].scale);
        this.group.position.set(this.mdSettings[0].posX, this.mdSettings[0].posY, this.mdSettings[0].posZ);
    ...
    addObjects1() {
      ...
      this.addSingleObject(this.mdSettings[1])
      .then((model) => {
        this.model2 = model.scene;
        this.group2.add(this.model2);
        this.group2.scale.set(this.mdSettings[1].scale, this.mdSettings[1].scale, this.mdSettings[1].scale);
        this.group2.position.set(this.mdSettings[1].posX, this.mdSettings[1].posY, this.mdSettings[1].posZ);
 ...
 addCylinder() {
  ...
  addFlame() {
    ...
    this.material3 = new THREE.ShaderMaterial({
      uniforms: {
        perlinnoise: { type: "t", value: new THREE.TextureLoader().load( "../static/src/assets/global/textures/flame/water-min.jpg") },
        color4: { value: new THREE.Vector3(64, 27, 0) },
        time: { type: "f", value: 0.0 },
        iResolution: { type: 'v3', value: new THREE.Vector3(0, this.width, this.height)},
        noise: { type: "t", value: new THREE.TextureLoader().load( "../static/src/assets/global/textures/flame/noise9.jpg") } },
        // wireframe:true,
...
/*postProc() {
  ...
  updateDraw(deltaTime) {
    ...
    this.material3.uniforms.color4.value = new THREE.Vector3(64, 27, 0);
  ...
  renderObject() {
    if(this.group && this.model1){
      ...
      var pos = this.path.getPoint(this.time);
          ...
          this.transform1(this.group);
          //this.group1.position.x = Math.sin( this.time * 2);
          //this.group1.position.y = Math.sin( this.time * 4)+ 0.2;
          //this.group1.position.z = Math.sin( this.time * 2 );
          //this.group1.rotation.x += ( this.targetRotation - this.group1.rotation.x )*0.005*this.time;
          this.group1.rotation.y += Math.sin(( this.targetRotation - this.group1.rotation.y))*0.05*this.time;
          this.model1.rotation.y += ( this.targetRotation - this.model1.rotation.y )*0.005*this.time;
     } 
  }
...
(function () {
    function init() {
      ...
      var mdSettings =
        [
          {
            path: '../static/src/assets/dataset/models/ufo/',
            mdName: 'ufospaceship.glb',
            scale: 1,
            posX: 0,
            posY: 0,
            posZ: 2,
      ...
      var UFO = new SpaceShift(o, mdSettings);
    }
